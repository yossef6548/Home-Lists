...
export default function Home() {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Background task will update the UI via SSE/SWR
    } catch (err) {
      console.error(err);
      refresh();
    } finally {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 transition-colors duration-200 pb-10">
      <div className="max-w-md mx-auto px-3 py-6">
        ...
        {/* Tab Switcher */}
        <div className="flex bg-zinc-100 dark:bg-zinc-900 p-1 rounded-xl mb-6 shadow-inner border border-zinc-200 dark:border-zinc-800">
          <button
            onClick={() => setTab("TASK")}
    ...
    <div className="space-y-4">
      ...
      {checkedItems.length > 0 && (
        <div className="mt-8 border-t border-zinc-100 dark:border-zinc-900 pt-6">
          <button 
            onClick={onClear}
            className="w-full py-2.5 text-xs font-black text-red-500 bg-red-50/50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30 rounded-lg active:scale-95 transition-all mb-4"
          >
            נקה משימות שהושלמו
          </button>
...
const ItemRow = memo(({ item, categories, onToggle, onDelete, onRename, onMove, isTask }: { 
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800/50 overflow-hidden transition-all">
      <div className="flex items-center gap-3 p-3">
        ...
        <div className="flex gap-0.5">
          <button onClick={() => onRename(item.id, item.name)} className="p-2 text-zinc-300 dark:text-zinc-600 active:text-blue-500">
          ...
          <button onClick={() => setShowMoveMenu(!showMoveMenu)} className="p-2 text-zinc-300 dark:text-zinc-600 active:text-blue-500">
          ...
          <button onClick={() => onDelete(item.id)} className="p-2 text-zinc-300 dark:text-zinc-600 active:text-red-500">
        ...
        <div className="p-2 bg-zinc-50 dark:bg-zinc-800/50 border-t border-zinc-100 dark:border-zinc-800">
          {isTask ? (
            <button 
              ...
              className="w-full p-2.5 bg-blue-600 text-white rounded-lg font-black text-xs shadow-sm active:scale-[0.98] transition-all flex items-center justify-center gap-2"
            >
            ...
            <div className="flex flex-col gap-2">
              <button 
                ...
                className="w-full p-2.5 bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-700 font-black text-xs active:bg-zinc-100"
              >
                העבר למשימות
              </button>
              <div className="space-y-1.5">
                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest px-1">בחר מיקום חדש:</p>
                <div className="flex flex-col gap-2 max-h-40 overflow-y-auto">
                  {stores.map(store => (
                    <div key={store.id} className="space-y-1 border-b border-zinc-100 dark:border-zinc-800 pb-1.5 last:border-0">
                      <p className="text-[8px] font-bold text-blue-500/60 px-1">{store.name}</p>
                      <div className="flex flex-wrap gap-1">
                        <button 
                          onClick={() => { onMove(item.id, "SHOPPING", store.id); setShowMoveMenu(false); }}
                          className="px-2 py-1 bg-white dark:bg-zinc-900 rounded border border-zinc-200 dark:border-zinc-700 font-bold text-[9px] active:border-blue-500"
                        >
                          כללי
                        </button>
    ...
    <div className="space-y-6">
      ...
      {items.some(i => i.isChecked) && (
        <div className="mt-8 border-t border-zinc-100 dark:border-zinc-900 pt-6">
          <button 
            onClick={onClear}
            className="w-full py-2.5 text-xs font-black text-red-500 bg-red-50/50 dark:bg-red-950/10 border border-red-100 dark:border-red-900/30 rounded-lg active:scale-95 transition-all mb-4"
          >
            נקה פריטים שנקנו
          </button>
          <div className="space-y-2 opacity-40 grayscale">
...
const CategoryView = memo(({ category, allCategories, allItems, onToggle, onDelete, onRename, onMove }: { 
  ...
  return (
    <div className="space-y-3 border-r-4 border-blue-500/10 pr-3">
      <h3 className="text-sm font-black text-zinc-800 dark:text-zinc-100 tracking-tight">{category.name}</h3>
      <div className="space-y-2">
        {items.map(item => (
        ...
        {visibleSubCategories.map(sub => (
          <div key={sub.id} className="space-y-2 pt-1">
            <h4 className="text-[9px] font-black text-blue-500/60 uppercase tracking-widest px-1">{sub.name}</h4>
